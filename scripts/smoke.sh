...
printf '1/82 readiness... '
curl -fsS "$BASE_URL/api/health/ready" >/dev/null
echo OK

printf '2/82 install and publish n8n workflow... '
...
printf '3/82 create organization... '
...
printf '4/82 create wedding template... '
...
printf '6/82 create event from template... '
...
printf '7/82 verify event plan regression... '
...
printf '8/82 create vendor catalog entry... '
...
printf '9/82 attach vendor to event... '
...
printf '10/82 request vendor confirmation... '
...
printf '11/82 verify n8n created and sent one outbound message... '
ATTEMPT=0
MESSAGE_ROW=''
...
printf '12/82 verify outbound idempotency... '
...
printf '13/82 simulate delivered through generic messaging webhook... '
post_mock_provider_status "$EXTERNAL_ID" delivered "$DELIVERED_AT"
echo OK

printf '14/82 verify delivered tracking... '
...
printf '15/82 simulate read through generic messaging webhook... '
post_mock_provider_status "$EXTERNAL_ID" read "$READ_AT"
echo OK

printf '16/82 verify read tracking... '
STATUS=$(docker compose exec -T postgres sh -c "psql -U \"\$POSTGRES_USER\" -d \"\$POSTGRES_DB\" -tAc \"SELECT status FROM outbound_messages WHERE id='$MESSAGE_ID'::uuid;\"" | tr -d '\r[:space:]')
[ "$STATUS" = 'read' ] || { echo "expected read, got $STATUS"; exit 1; }
echo OK

printf '17/82 verify provider webhook idempotency... '
post_mock_provider_status "$EXTERNAL_ID" read "$READ_AT"
...
printf '18/82 simulate supplier inbound confirmation... '
...
printf '19/82 verify inbound message persisted and correlated... '
ATTEMPT=0
INBOUND_ROW=''
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '20/82 verify supplier response updated vendor assignment... '
...
printf '21/82 verify inbound webhook/process idempotency... '
post_mock_inbound_message "$INBOUND_EXTERNAL_ID" '5521999999999' 'Sim, confirmado. Chegaremos às 14:30 com 3 pessoas.' "$INBOUND_AT"
...
printf '22/82 verify event activity timeline... '
ATTEMPT=0
ACTIVITY_JSON=''
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '23/82 verify activity projection idempotency... '
...
printf '24/82 create two pending confirmation contexts for ambiguity... '
...
printf '25/82 simulate ambiguous supplier response... '
...
printf '26/82 verify operational inbox item... '
ATTEMPT=0
INBOX_JSON=''
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '27/82 resolve inbox item... '
...
printf '28/82 select planner event context through rule-based command... '
...
printf '29/82 create task from conversational context... '
...
printf '30/82 verify command-created task traceability... '
...
printf '31/82 query event status using saved conversation context... '
...
printf '32/82 complete task through command engine... '
...
printf '33/82 add event note through command engine... '
...
printf '34/82 verify command activity projection... '
ATTEMPT=0
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '35/82 reject sensitive change without mutating event... '
...
printf '36/82 verify command idempotency... '
...
printf '37/82 verify persisted conversation context... '
...
printf '38/82 operational agent workspace overview through deterministic smoke provider... '
...
printf '39/82 operational agent creates task through structured CommandEngine delegation... '
...
printf '40/82 verify agent write used CommandEngine and domain task path... '
...
printf '41/82 verify operational agent turn idempotency... '
...
printf '42/82 verify operational agent conversation history... '
...
printf '43/82 operational agent creates sensitive time change proposal... '
...
printf '44/82 verify proposal impacts and approval inbox projection... '
...
printf '45/82 approve pending proposal through conversational follow-up... '
...
printf '46/82 verify approval idempotency... '
...
printf '47/82 verify change activity and inbox resolution... '
ATTEMPT=0
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '48/82 wait for dependency evaluation after applied time change... '
ATTEMPT=0
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '49/82 verify dependency API and inbox projection... '
...
printf '50/82 apply safe dependency suggestions through conversational agent... '
...
printf '51/82 verify dependency suggestion updated vendor schedule... '
...
printf '52/82 verify dependency activity and inbox resolution... '
ATTEMPT=0
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '53/82 create overdue critical task for risk evaluation... '
...
printf '54/82 wait for Risk Engine to detect overdue task... '
ATTEMPT=0
RISK_ID=''
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '55/82 verify risk API and high-risk inbox projection... '
...
printf '56/82 query event risks through Operational Agent... '
...
printf '57/82 wait for Health Score degradation and query API... '
ATTEMPT=0
...
printf '58/82 query Health Score through Operational Agent... '
...
printf '59/82 verify manual Health Score evaluation idempotency... '
...
printf '60/82 acknowledge risk without resolving its cause... '
...
printf '61/82 complete underlying task and trigger automatic risk resolution... '
...
printf '62/82 verify risk, inbox and Health Score auto-reconciled... '
ATTEMPT=0
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '63/82 verify Health Score history and activity... '
...
printf '64/82 configure scheduled Daily Brief through Operational Agent... '
...
printf '65/82 wait for scheduled Daily Brief and WhatsApp delivery... '
ATTEMPT=0
BRIEF_ROW=''
BRIEF_MESSAGE_ROW=''
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '66/82 verify scheduled Daily Brief idempotency... '
sleep 2
...
printf '67/82 verify Daily Brief settings and today API... '
...
printf '68/82 query Daily Brief through Operational Agent... '
...
printf '69/82 verify manual Daily Brief revision and idempotency... '
...
printf '70/82 create D-1 smoke event for tomorrow... '
...
printf '71/82 configure D-1 schedule independently through Operational Agent... '
...
printf '72/82 wait for scheduled D-1 briefing and WhatsApp delivery... '
ATTEMPT=0
D1_BRIEF_ROW=''
D1_MESSAGE_ROW=''
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '73/82 verify scheduled D-1 idempotency... '
sleep 2
...
printf '74/82 verify independent D-1 schedule settings API... '
...
printf '75/82 query D-1 readiness through Operational Agent... '
...
printf '76/82 verify D-1 deterministic readiness and timeline API... '
...
printf '77/82 verify manual D-1 revision, idempotency and history... '
...
printf '78/82 create guest-count proposal through REST API... '
...
printf '79/82 reject REST proposal without mutating guest count... '
...
printf '80/82 verify workspace Health Score ranking... '
...
printf '81/82 verify event resource exposes persisted Health Score... '
...
printf '82/82 verify all generated domain events were acknowledged... ' 
ATTEMPT=0
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
