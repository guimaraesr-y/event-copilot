...
printf '1/62 readiness... '
curl -fsS "$BASE_URL/api/health/ready" >/dev/null
echo OK

printf '2/62 install and publish n8n workflow... '
...
printf '3/62 create organization... '
...
printf '4/62 create wedding template... '
...
printf '6/62 create event from template... '
...
printf '7/62 verify event plan regression... '
...
printf '8/62 create vendor catalog entry... '
...
printf '9/62 attach vendor to event... '
...
printf '10/62 request vendor confirmation... '
...
printf '11/62 verify n8n created and sent one outbound message... '
ATTEMPT=0
MESSAGE_ROW=''
...
printf '12/62 verify outbound idempotency... '
...
printf '13/62 simulate delivered through generic messaging webhook... '
post_mock_provider_status "$EXTERNAL_ID" delivered "$DELIVERED_AT"
echo OK

printf '14/62 verify delivered tracking... '
...
printf '15/62 simulate read through generic messaging webhook... '
post_mock_provider_status "$EXTERNAL_ID" read "$READ_AT"
echo OK

printf '16/62 verify read tracking... '
STATUS=$(docker compose exec -T postgres sh -c "psql -U \"\$POSTGRES_USER\" -d \"\$POSTGRES_DB\" -tAc \"SELECT status FROM outbound_messages WHERE id='$MESSAGE_ID'::uuid;\"" | tr -d '\r[:space:]')
[ "$STATUS" = 'read' ] || { echo "expected read, got $STATUS"; exit 1; }
echo OK

printf '17/62 verify provider webhook idempotency... '
post_mock_provider_status "$EXTERNAL_ID" read "$READ_AT"
...
printf '18/62 simulate supplier inbound confirmation... '
...
printf '19/62 verify inbound message persisted and correlated... '
ATTEMPT=0
INBOUND_ROW=''
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '20/62 verify supplier response updated vendor assignment... '
...
printf '21/62 verify inbound webhook/process idempotency... '
post_mock_inbound_message "$INBOUND_EXTERNAL_ID" '5521999999999' 'Sim, confirmado. Chegaremos às 14:30 com 3 pessoas.' "$INBOUND_AT"
...
printf '22/62 verify event activity timeline... '
ATTEMPT=0
ACTIVITY_JSON=''
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '23/62 verify activity projection idempotency... '
...
printf '24/62 create two pending confirmation contexts for ambiguity... '
...
printf '25/62 simulate ambiguous supplier response... '
...
printf '26/62 verify operational inbox item... '
ATTEMPT=0
INBOX_JSON=''
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '27/62 resolve inbox item... '
...
printf '28/62 select planner event context through rule-based command... '
...
printf '29/62 create task from conversational context... '
...
printf '30/62 verify command-created task traceability... '
...
printf '31/62 query event status using saved conversation context... '
...
printf '32/62 complete task through command engine... '
...
printf '33/62 add event note through command engine... '
...
printf '34/62 verify command activity projection... '
ATTEMPT=0
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '35/62 reject sensitive change without mutating event... '
...
printf '36/62 verify command idempotency... '
...
printf '37/62 verify persisted conversation context... '
...
printf '38/62 operational agent workspace overview through deterministic smoke provider... '
...
printf '39/62 operational agent creates task through structured CommandEngine delegation... '
...
printf '40/62 verify agent write used CommandEngine and domain task path... '
...
printf '41/62 verify operational agent turn idempotency... '
...
printf '42/62 verify operational agent conversation history... '
...
printf '43/62 operational agent creates sensitive time change proposal... '
...
printf '44/62 verify proposal impacts and approval inbox projection... '
...
printf '45/62 approve pending proposal through conversational follow-up... '
...
printf '46/62 verify approval idempotency... '
...
printf '47/62 verify change activity and inbox resolution... '
ATTEMPT=0
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '48/62 wait for dependency evaluation after applied time change... '
ATTEMPT=0
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '49/62 verify dependency API and inbox projection... '
...
printf '50/62 apply safe dependency suggestions through conversational agent... '
...
printf '51/62 verify dependency suggestion updated vendor schedule... '
...
printf '52/62 verify dependency activity and inbox resolution... '
ATTEMPT=0
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '53/62 create overdue critical task for risk evaluation... '
...
printf '54/62 wait for Risk Engine to detect overdue task... '
ATTEMPT=0
RISK_ID=''
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '55/62 verify risk API and high-risk inbox projection... '
...
printf '56/62 query event risks through Operational Agent... '
...
printf '57/62 acknowledge risk without resolving its cause... '
...
printf '58/62 complete underlying task and trigger automatic risk resolution... '
...
printf '59/62 verify risk and inbox auto-resolved... '
ATTEMPT=0
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '60/62 create guest-count proposal through REST API... '
...
printf '61/62 reject REST proposal without mutating guest count... '
...
printf '62/62 verify all generated domain events were acknowledged... ' 
ATTEMPT=0
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
