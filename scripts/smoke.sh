...
printf '4/50 create wedding template... '
...
printf '6/50 create event from template... '
...
printf '7/50 verify event plan regression... '
...
printf '8/50 create vendor catalog entry... '
...
printf '9/50 attach vendor to event... '
...
printf '10/50 request vendor confirmation... '
...
printf '11/50 verify n8n created and sent one outbound message... '
ATTEMPT=0
MESSAGE_ROW=''
...
printf '12/50 verify outbound idempotency... '
...
printf '13/50 simulate delivered through generic messaging webhook... '
post_mock_provider_status "$EXTERNAL_ID" delivered "$DELIVERED_AT"
echo OK

printf '14/50 verify delivered tracking... '
...
printf '15/50 simulate read through generic messaging webhook... '
post_mock_provider_status "$EXTERNAL_ID" read "$READ_AT"
echo OK

printf '16/50 verify read tracking... '
STATUS=$(docker compose exec -T postgres sh -c "psql -U \"\$POSTGRES_USER\" -d \"\$POSTGRES_DB\" -tAc \"SELECT status FROM outbound_messages WHERE id='$MESSAGE_ID'::uuid;\"" | tr -d '\r[:space:]')
[ "$STATUS" = 'read' ] || { echo "expected read, got $STATUS"; exit 1; }
echo OK

printf '17/50 verify provider webhook idempotency... '
post_mock_provider_status "$EXTERNAL_ID" read "$READ_AT"
...
printf '18/50 simulate supplier inbound confirmation... '
...
printf '19/50 verify inbound message persisted and correlated... '
ATTEMPT=0
INBOUND_ROW=''
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '20/50 verify supplier response updated vendor assignment... '
...
printf '21/50 verify inbound webhook/process idempotency... '
post_mock_inbound_message "$INBOUND_EXTERNAL_ID" '5521999999999' 'Sim, confirmado. Chegaremos às 14:30 com 3 pessoas.' "$INBOUND_AT"
...
printf '22/50 verify event activity timeline... '
ATTEMPT=0
ACTIVITY_JSON=''
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '23/50 verify activity projection idempotency... '
...
printf '24/50 create two pending confirmation contexts for ambiguity... '
...
printf '25/50 simulate ambiguous supplier response... '
...
printf '26/50 verify operational inbox item... '
ATTEMPT=0
INBOX_JSON=''
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '27/50 resolve inbox item... '
...
printf '28/50 select planner event context through rule-based command... '
...
printf '29/50 create task from conversational context... '
...
printf '30/50 verify command-created task traceability... '
...
printf '31/50 query event status using saved conversation context... '
...
printf '32/50 complete task through command engine... '
...
printf '33/50 add event note through command engine... '
...
printf '34/50 verify command activity projection... '
ATTEMPT=0
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '35/50 reject sensitive change without mutating event... '
...
printf '36/50 verify command idempotency... '
...
printf '37/50 verify persisted conversation context... '
...
printf '38/50 operational agent workspace overview through deterministic smoke provider... '
...
printf '39/50 operational agent creates task through structured CommandEngine delegation... '
...
printf '40/50 verify agent write used CommandEngine and domain task path... '
...
printf '41/50 verify operational agent turn idempotency... '
...
printf '42/50 verify operational agent conversation history... '
...
printf '43/50 operational agent creates sensitive time change proposal... '
...
printf '44/50 verify proposal impacts and approval inbox projection... '
...
printf '45/50 approve pending proposal through conversational follow-up... '
...
printf '46/50 verify approval idempotency... '
...
printf '47/50 verify change activity and inbox resolution... '
ATTEMPT=0
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '48/50 create guest-count proposal through REST API... '
...
printf '49/50 reject REST proposal without mutating guest count... '
...
printf '50/50 verify all generated domain events were acknowledged... ' 
ATTEMPT=0
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
