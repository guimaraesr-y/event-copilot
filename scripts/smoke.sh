...
printf '1/19 readiness... '
curl -fsS "$BASE_URL/api/health/ready" >/dev/null
echo OK

printf '2/19 install and publish n8n workflow... '
./scripts/n8n-sync.sh >/dev/null
echo OK

printf '3/19 create organization... '
...
printf '4/19 create wedding template... '
...
printf '6/19 create event from template... '
...
printf '7/19 verify event plan regression... '
...
printf '8/19 create vendor catalog entry... '
...
printf '9/19 attach vendor to event... '
...
printf '10/19 request vendor confirmation... '
...
printf '11/19 verify n8n created and sent one outbound message... '
ATTEMPT=0
MESSAGE_ROW=''
...
printf '12/19 verify outbound idempotency... '
...
printf '13/19 simulate delivered through generic messaging webhook... '
post_mock_provider_status "$EXTERNAL_ID" delivered "$DELIVERED_AT"
echo OK

printf '14/19 verify delivered tracking... '
...
printf '15/19 simulate read through generic messaging webhook... '
post_mock_provider_status "$EXTERNAL_ID" read "$READ_AT"
echo OK

printf '16/19 verify read tracking... '
STATUS=$(docker compose exec -T postgres sh -c "psql -U \"\$POSTGRES_USER\" -d \"\$POSTGRES_DB\" -tAc \"SELECT status FROM outbound_messages WHERE id='$MESSAGE_ID'::uuid;\"" | tr -d '\r[:space:]')
[ "$STATUS" = 'read' ] || { echo "expected read, got $STATUS"; exit 1; }
echo OK

printf '17/19 verify provider webhook idempotency... '
post_mock_provider_status "$EXTERNAL_ID" read "$READ_AT"
...
printf '18/19 preserve vendor confirmation regression... '
...
printf '19/19 verify all generated domain events were acknowledged... '
ATTEMPT=0
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
