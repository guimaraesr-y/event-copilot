...
printf '1/28 readiness... '
curl -fsS "$BASE_URL/api/health/ready" >/dev/null
echo OK

printf '2/28 install and publish n8n workflow... '
./scripts/n8n-sync.sh >/dev/null
echo OK

printf '3/28 create organization... '
...
printf '4/28 create wedding template... '
...
printf '6/28 create event from template... '
...
printf '7/28 verify event plan regression... '
...
printf '8/28 create vendor catalog entry... '
...
printf '9/28 attach vendor to event... '
...
printf '10/28 request vendor confirmation... '
...
printf '11/28 verify n8n created and sent one outbound message... '
ATTEMPT=0
MESSAGE_ROW=''
...
printf '12/28 verify outbound idempotency... '
...
printf '13/28 simulate delivered through generic messaging webhook... '
post_mock_provider_status "$EXTERNAL_ID" delivered "$DELIVERED_AT"
echo OK

printf '14/28 verify delivered tracking... '
...
printf '15/28 simulate read through generic messaging webhook... '
post_mock_provider_status "$EXTERNAL_ID" read "$READ_AT"
echo OK

printf '16/28 verify read tracking... '
STATUS=$(docker compose exec -T postgres sh -c "psql -U \"\$POSTGRES_USER\" -d \"\$POSTGRES_DB\" -tAc \"SELECT status FROM outbound_messages WHERE id='$MESSAGE_ID'::uuid;\"" | tr -d '\r[:space:]')
[ "$STATUS" = 'read' ] || { echo "expected read, got $STATUS"; exit 1; }
echo OK

printf '17/28 verify provider webhook idempotency... '
post_mock_provider_status "$EXTERNAL_ID" read "$READ_AT"
...
printf '18/28 simulate supplier inbound confirmation... '
...
printf '19/28 verify inbound message persisted and correlated... '
ATTEMPT=0
INBOUND_ROW=''
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '20/28 verify supplier response updated vendor assignment... '
...
printf '21/28 verify inbound webhook/process idempotency... '
post_mock_inbound_message "$INBOUND_EXTERNAL_ID" '5521999999999' 'Sim, confirmado. Chegaremos às 14:30 com 3 pessoas.' "$INBOUND_AT"
...
printf '22/28 verify event activity timeline... '
ATTEMPT=0
ACTIVITY_JSON=''
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '23/28 verify activity projection idempotency... '
...
printf '24/28 create two pending confirmation contexts for ambiguity... '
...
printf '25/28 simulate ambiguous supplier response... '
...
printf '26/28 verify operational inbox item... '
ATTEMPT=0
INBOX_JSON=''
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '27/28 resolve inbox item... '
...
printf '28/28 verify all generated domain events were acknowledged... '
ATTEMPT=0
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
