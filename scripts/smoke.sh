...
printf '1/43 readiness... '
curl -fsS "$BASE_URL/api/health/ready" >/dev/null
echo OK

printf '2/43 install and publish n8n workflow... '
./scripts/n8n-sync.sh >/dev/null
echo OK

printf '3/43 create organization... '
...
printf '4/43 create wedding template... '
...
printf '6/43 create event from template... '
...
printf '7/43 verify event plan regression... '
...
printf '8/43 create vendor catalog entry... '
...
printf '9/43 attach vendor to event... '
...
printf '10/43 request vendor confirmation... '
...
printf '11/43 verify n8n created and sent one outbound message... '
ATTEMPT=0
MESSAGE_ROW=''
...
printf '12/43 verify outbound idempotency... '
...
printf '13/43 simulate delivered through generic messaging webhook... '
post_mock_provider_status "$EXTERNAL_ID" delivered "$DELIVERED_AT"
echo OK

printf '14/43 verify delivered tracking... '
...
printf '15/43 simulate read through generic messaging webhook... '
post_mock_provider_status "$EXTERNAL_ID" read "$READ_AT"
echo OK

printf '16/43 verify read tracking... '
STATUS=$(docker compose exec -T postgres sh -c "psql -U \"\$POSTGRES_USER\" -d \"\$POSTGRES_DB\" -tAc \"SELECT status FROM outbound_messages WHERE id='$MESSAGE_ID'::uuid;\"" | tr -d '\r[:space:]')
[ "$STATUS" = 'read' ] || { echo "expected read, got $STATUS"; exit 1; }
echo OK

printf '17/43 verify provider webhook idempotency... '
post_mock_provider_status "$EXTERNAL_ID" read "$READ_AT"
...
printf '18/43 simulate supplier inbound confirmation... '
...
printf '19/43 verify inbound message persisted and correlated... '
ATTEMPT=0
INBOUND_ROW=''
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '20/43 verify supplier response updated vendor assignment... '
...
printf '21/43 verify inbound webhook/process idempotency... '
post_mock_inbound_message "$INBOUND_EXTERNAL_ID" '5521999999999' 'Sim, confirmado. Chegaremos às 14:30 com 3 pessoas.' "$INBOUND_AT"
...
printf '22/43 verify event activity timeline... '
ATTEMPT=0
ACTIVITY_JSON=''
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '23/43 verify activity projection idempotency... '
...
printf '24/43 create two pending confirmation contexts for ambiguity... '
...
printf '25/43 simulate ambiguous supplier response... '
...
printf '26/43 verify operational inbox item... '
ATTEMPT=0
INBOX_JSON=''
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '27/43 resolve inbox item... '
...
printf '28/43 select planner event context through rule-based command... '
...
printf '29/43 create task from conversational context... '
...
printf '30/43 verify command-created task traceability... '
...
printf '31/43 query event status using saved conversation context... '
...
printf '32/43 complete task through command engine... '
...
printf '33/43 add event note through command engine... '
...
printf '34/43 verify command activity projection... '
ATTEMPT=0
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
...
printf '35/43 reject sensitive change without mutating event... '
...
printf '36/43 verify command idempotency... '
...
printf '37/43 verify persisted conversation context... '
...
printf '38/43 operational agent workspace overview through deterministic smoke provider... '
...
printf '39/43 operational agent creates task through structured CommandEngine delegation... '
...
printf '40/43 verify agent write used CommandEngine and domain task path... '
...
printf '41/43 verify operational agent turn idempotency... '
...
printf '42/43 verify operational agent conversation history... '
...
printf '43/43 verify all generated domain events were acknowledged... '
ATTEMPT=0
while [ "$ATTEMPT" -lt "$OUTBOX_WAIT_SECONDS" ]; do
