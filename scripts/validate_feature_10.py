...
if version!='0.10.0': fail(f'version expected 0.10.0, got {version}')
...
for n in ['013_operational_agent_openrouter','migration013OperationalAgentOpenRouter']:
  if n not in text('packages/database/src/migrate.ts'): fail(f'migrate missing {n}')
for n in ['014_change_proposals','migration014ChangeProposals']:
  if n not in text('packages/database/src/migrate.ts'): fail(f'migrate missing {n}')
for n in ["createTable('change_proposals')","createTable('change_proposal_impacts')",'change_proposals_org_idempotency_unique']:
  if n not in text('packages/database/src/migrations/014_change_proposals.ts'): fail(f'migration 014 missing {n}')
...
for n in ['get_workspace_overview','get_event_details','get_event_activity','get_inbox','create_task','get_change_proposals','propose_event_time_change','approve_change_proposal','reject_change_proposal','executeStructuredToolCommand','OPERATIONAL_AGENT']:
...
for n in ['/api/v1/events/:eventId/change-proposals','/api/v1/change-proposals/:id/approve','/api/v1/change-proposals/:id/reject']:
  if n not in text('apps/api/src/routes/change-proposals.ts'): fail(f'change proposal route missing {n}')
for n in ['ChangeProposalEngine','change.proposed','change.applied','change.rejected']:
  if n not in text('packages/event-engine/src/change-proposal-engine.ts'): fail(f'change proposal engine missing {n}')
...
for n in ['operational agent workspace overview','operational agent creates task','operational agent turn idempotency','operational agent conversation history','operational agent creates sensitive time change proposal','approve pending proposal through conversational follow-up','reject REST proposal without mutating guest count','50/50 verify all generated domain events']:
...
print('\nMini-feature 10 Change Proposals validation passed.')
