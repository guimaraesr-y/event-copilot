...
import type { EventDayEngine } from './event-day-engine.ts'
import type {
...
const TOOLS: AgentToolDefinition[] = [
  ...
  {
    name: 'get_event_day_status',
    description: 'Get the deterministic live Event Day snapshot for one event: operational status, supplier arrivals, open tasks, timeline and next actions. Use for questions about how the event is going right now.',
    parameters: objectSchema({ eventId: stringSchema('Exact event UUID from the event catalog.') }, ['eventId']),
  },
  {
  ...
  {
    name: 'mark_event_day_vendor_arrived',
    description: 'Record a supplier/vendor actual arrival during Event Day. Only call when the CURRENT user explicitly states that the supplier arrived. vendorReference is the human name/category mentioned by the user.',
    parameters: objectSchema({ eventId: stringSchema('Exact event UUID.'), vendorReference: stringSchema('Vendor name or category exactly as referenced by the user.') }, ['eventId','vendorReference']),
  },
  {
    name: 'mark_event_day_vendor_departed',
    description: 'Record a supplier/vendor actual departure during Event Day. Only call when the CURRENT user explicitly states that the supplier left/departed.',
    parameters: objectSchema({ eventId: stringSchema('Exact event UUID.'), vendorReference: stringSchema('Vendor name or category exactly as referenced by the user.') }, ['eventId','vendorReference']),
  },
  {
    name: 'complete_event_day',
    description: 'Complete Event Day Mode and mark the event completed. Only call when the CURRENT user explicitly asks to finish/close the event operation.',
    parameters: objectSchema({ eventId: stringSchema('Exact event UUID from the event catalog.') }, ['eventId']),
  },
  {
  ...
  private async executeTool(
    ...
    switch (call.name) {
      ...
      case 'start_event_day': {
        assertNoUnexpectedKeys(call.arguments,['eventId'])
        if(!isExplicitEventDayStart(input.text))throw new OperationalAgentValidationError('Current user message does not explicitly ask to start Event Day')
        const event=requireEvent(events,requiredString(call.arguments,'eventId'))
        const result=await this.deps.eventDayEngine.start({organizationId:input.organizationId,eventId:event.id,sender:input.sender})
        return {reply:`Event Day de ${event.name} iniciado. Estado operacional: ${eventDayStatusLabel(result.snapshot.operationalStatus)}.`,duplicate:result.duplicate,eventDay:serializeEventDayForAgent(result.snapshot)}
      }
      case 'mark_event_day_vendor_arrived': {
        assertNoUnexpectedKeys(call.arguments,['eventId','vendorReference'])
        if(!isExplicitVendorArrival(input.text))throw new OperationalAgentValidationError('Current user message does not explicitly state that a vendor arrived')
        const event=requireEvent(events,requiredString(call.arguments,'eventId'))
        const current=await this.deps.eventDayEngine.get(input.organizationId,event.id)
        const vendor=resolveEventDayVendor(current,requiredString(call.arguments,'vendorReference'))
        const result=await this.deps.eventDayEngine.markVendorArrived({organizationId:input.organizationId,eventId:event.id,eventVendorId:vendor.eventVendorId,sender:input.sender})
        return {reply:`Chegada de ${vendor.vendorName} registrada agora. Estado operacional: ${eventDayStatusLabel(result.snapshot.operationalStatus)}.`,duplicate:result.duplicate,eventDay:serializeEventDayForAgent(result.snapshot)}
      }
      case 'mark_event_day_vendor_departed': {
        assertNoUnexpectedKeys(call.arguments,['eventId','vendorReference'])
        if(!isExplicitVendorDeparture(input.text))throw new OperationalAgentValidationError('Current user message does not explicitly state that a vendor departed')
        const event=requireEvent(events,requiredString(call.arguments,'eventId'))
        const current=await this.deps.eventDayEngine.get(input.organizationId,event.id)
        const vendor=resolveEventDayVendor(current,requiredString(call.arguments,'vendorReference'))
