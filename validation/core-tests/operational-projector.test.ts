...
projection = projector.project(event('risk.detected', { eventId, title: 'Buffet ainda não confirmou', description: 'Confirmação pendente.', severity: 'high', score: 72 }))
ok(projection.activity?.category === 'risk' && projection.inbox?.type === 'operational_risk' && projection.inbox.severity === 'warning', 'high risk becomes activity and inbox')
projection = projector.project(event('risk.detected', { eventId, title: 'Risco baixo', severity: 'low', score: 18 }))
ok(projection.activity?.action === 'risk.detected' && projection.inbox === null, 'low risk stays out of inbox')
projection = projector.project(event('risk.updated', { eventId, title: 'Risco escalou', previousSeverity: 'medium', severity: 'critical', score: 88 }))
