...

{
  ...
  const compactTime = await provider.complete({ messages: [{ role:'user', content:'configure meu brief diario pa 21h50 todos os dias' }], tools })
  assert(compactTime.toolCalls[0]?.name==='configure_daily_brief','deterministic provider routes recurring brief configuration')
  assert(compactTime.toolCalls[0]?.arguments.enabled===true&&compactTime.toolCalls[0]?.arguments.localTime==='21:50','deterministic provider understands daily activation and 21h50 notation')
  const disabled = await provider.complete({ messages: [{ role:'user', content:'desative meu brief diario' }], tools })
  assert(disabled.toolCalls[0]?.arguments.enabled===false,'deterministic provider only disables on explicit disable language')
  const timeOnly = await provider.complete({ messages: [{ role:'user', content:'mude apenas o horario do brief diario para 22h10' }], tools })
  assert(timeOnly.toolCalls[0]?.arguments.enabled===undefined&&timeOnly.toolCalls[0]?.arguments.localTime==='22:10','deterministic provider preserves enabled state for time-only edits')
