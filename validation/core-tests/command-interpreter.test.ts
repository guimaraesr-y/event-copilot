import { AICommandInterpreter, RuleBasedCommandInterpreter } from '../../packages/event-engine/src/command-interpreter.ts'
...
}
{
  ...
  const interpreter=new AICommandInterpreter({apiKey:'test-key',model:'gpt-test',fetchImpl:fakeFetch})
  const result=await interpreter.interpret({...input,text:'Como está o casamento da Ana?'})
  assert(result.intent==='GET_EVENT_STATUS','AI interpreter parses structured result')
  assert(captured.model==='gpt-test' && captured.store===false,'AI request uses configured Responses model without storage')
  assert(captured.text?.format?.type==='json_schema' && captured.text?.format?.strict===true,'AI request uses strict Structured Outputs')
  assert(captured.text?.format?.schema?.additionalProperties===false,'schema forbids unexpected properties')
}
{
  const fakeFetch:typeof fetch=async()=>new Response(JSON.stringify({status:'completed',output:[]}),{status:200})
  const interpreter=new AICommandInterpreter({apiKey:'test-key',fetchImpl:fakeFetch})
  let failed=false
  try{await interpreter.interpret(input)}catch{failed=true}
  assert(failed,'AI interpreter rejects missing structured output')
}

console.log('CommandInterpreter: 4/4 behavioral scenarios passed')
